...
export default function CEEmpire() {
  ...
  return (
    <div style={{ minHeight: "100vh", fontFamily: '"Prompt","Noto Sans Thai","Inter",sans-serif' }}>
      ...
      <header className="glass" style={{ position: "sticky", top: 10, zIndex: 40, borderRadius: 26, padding: "11px 14px", margin: "10px 12px 0", display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
          ...
          <div style={{ width: 56, height: 56, borderRadius: 18, display: "grid", placeItems: "center", position: "relative", background: "linear-gradient(135deg, rgba(56,241,255,.28), rgba(37,99,255,.22) 38%, rgba(139,92,255,.18) 62%, rgba(255,215,106,.24)), rgba(5,8,22,.86)", border: "1px solid rgba(56,241,255,.52)", boxShadow: "0 0 26px rgba(56,241,255,.26), 0 0 22px rgba(255,215,106,.14), inset 0 1px 0 rgba(255,255,255,.16)" }}>
            ...
            <span style={{ fontFamily: '"Orbitron","Inter",sans-serif', fontSize: 15, fontWeight: 900, letterSpacing: "-.08em", color: "#F8FDFF", textShadow: "0 0 12px rgba(56,241,255,.95), 0 0 16px rgba(255,215,106,.50)" }}>CE</span>
          </div>
          <div>
            <div style={{ fontFamily: '"Orbitron","Inter",sans-serif', fontSize: 20, fontWeight: 950, letterSpacing: "-.03em" }}>CE Empire</div>
            <div style={{ fontSize: 11, color: "#B7C6DE", letterSpacing: ".04em" }}>Banking Management System</div>
          </div>
        </div>
      ...
      <section className="glass" style={{ margin: "12px 12px 0", borderRadius: 28, padding: 16, position: "relative", overflow: "hidden" }}>
        ...
        <div style={{ position: "relative", zIndex: 1, display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
          <div>
            ...
            <div style={{ fontSize: 12, color: "#AFC0D8", lineHeight: 1.45 }}>ระบบจัดการบัญชีธนาคาร • ติดตามรายจ่าย • ทีมงาน</div>
          </div>
            ...
            <div className="animate-fade-up">
              <GlassCard>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(160px, 1fr))", gap: 12, marginBottom: 16 }}>
                  <MetricCard label="บัญชีทั้งหมด" value={stats?.totalAccounts ?? 0} icon={Building2} color="cyan" />
                  <MetricCard label="ยอดจ่ายแล้ว" value={`฿${(stats?.totalPaid ?? 0).toLocaleString()}`} icon={CheckCircle} color="green" />
                  <MetricCard label="ยอดค้างจ่าย" value={`฿${(stats?.totalDue ?? 0).toLocaleString()}`} icon={AlertCircle} color="gold" />
                  <MetricCard label="สลิปหลักฐาน" value={stats?.totalProof ?? 0} icon={FileText} color="violet" />
                </div>
      ...
      {/* FAB */}
      <button onClick={() => { setEditAccount(null); setShowAccountForm(true); }} className="btn-primary" style={{ position: "fixed", right: 16, bottom: 80, zIndex: 60, borderRadius: 999, padding: "14px 18px", boxShadow: "0 18px 50px rgba(56,241,255,.26), 0 0 28px rgba(37,99,255,.20)", cursor: "pointer", display: "flex", alignItems: "center", gap: 8, fontWeight: 900, fontSize: 14 }}>
        <Plus size={18} /> เพิ่ม
      </button>
